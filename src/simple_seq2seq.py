...
class SimpleSeq2Seq(Model):
    ...
    def __init__(self,
        ...
        if use_bleu:
            ...
            #self._bleu = BLEU(exclude_indices={pad_index, self._end_index, self._start_index}, mode=mode, vocab=vocab)
    ...
    def init_weights(self):
        ...
        for name, param in self._decoder_cell.named_parameters():
            if 'bias' in name:
                nn.init.constant_(param, 0.0)
            elif 'weight' in name:
                nn.init.xavier_normal_(param)

        for name, param in self._encoder._module.named_parameters():
            if 'bias' in name:
                nn.init.constant_(param, 0.0)
            elif 'weight' in name:
                nn.init.xavier_normal_(param)
                
        self._output_projection_layer.bias.data.fill_(0)
        self._output_projection_layer.weight.data.uniform_(-0.1, 0.1)
