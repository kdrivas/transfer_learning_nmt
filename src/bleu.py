...
def pad_to_dense(M):
    ...
    Z = np.zeros((len(M), maxlen), dtype=np.float)
...
class BLEU(Metric):
    ...
    def __call__(self,  # type: ignore
        ...
        if self._mode == 'char' or self._mode == 'syl':
            subword_matrix = predictions.data.numpy()
            aux_matrix = []
            d = self._vocab.get_index_to_token_vocabulary('target_tokens')
            pos = 0
            for subword_pred in subword_matrix:
                aux_pred = []
                text_temp = []
                word = ''
                last_subword = ''
                last = False
                start_token_word = False
                for subword in subword_pred:
                    if d[subword] == '@@':
                        if len(last_subword):
                            aux_pred.append(float(last_subword))
                            text_temp.append(word)
                        last = False
                    elif d[subword].startswith('@') and d[subword].endswith('@'):
                        if last:
                            aux_pred.append(float(last_subword))
                            text_temp.append(word)
                            last = False
                        aux_pred.append(float(subword))
                        text_temp.append(d[subword])
                    else:
                        if last:
                            last_subword += str(subword)
                            word += d[subword]
                        else:
                            last_subword = str(subword)
                            word = d[subword]
                        last = True
                aux_matrix.append(aux_pred)
                if pos < 1:
                    print('hypothesis:', text_temp)
                pos += 1
                #print(text_temp)
            predictions = torch.LongTensor(pad_to_dense(aux_matrix))

            subword_matrix = gold_targets.data.numpy()
            aux_matrix = []
            pos = 0
            for subword_pred in subword_matrix:
                aux_pred = []
                ...
                pos += 1
                #print(text_temp)
                #print(len(text_temp) == len(aux_pred))
            #print(lalalala)
            gold_targets = torch.LongTensor(pad_to_dense(aux_matrix))
        else:
            subword_matrix = predictions.data.numpy()
            aux_matrix = []
            d = self._vocab.get_index_to_token_vocabulary('target_tokens')
            ...
            for subword_pred in subword_matrix:
                ...
                last = False
                for subword in subword_pred:
                    if subword in d and not d[subword].startswith('@') and d[subword].endswith('@@'):
                        if last:
                            last_subword += str(subword)
                            word += d[subword]
                        else:
                        ...
                        if last:
                            aux_pred.append(-float(last_subword + str(subword)))
                            text_temp.append(word + d[subword])
                            last = False
                        else:
                            aux_pred.append(float(subword))
                            text_temp.append(d[subword])
